/*
	...
	On Bluedroid builds (C3, S3, …) this is a transparent pass-through that
	includes the real esp_bt / esp_gap / esp_gatt headers — no behavioural
	change vs. before this file existed.

	On NimBLE builds (C6) the Bluedroid headers are not available, so we
	define just enough of their public type/macro vocabulary here for
	custom_ble.h and lispif_ble_extensions.c to compile unchanged. The
	concrete translation to NimBLE types happens inside
	custom_ble_nimble.c / comm_ble_nimble.c.

	This header should ONLY be included by code that wants Bluedroid-shaped
	types in its public API (custom_ble.h, lispif_ble_extensions.c). Real
	Bluedroid implementation files include the esp_bt headers directly.
*/
