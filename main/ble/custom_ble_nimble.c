/*
	NimBLE port of custom_ble.c (Stage 3).

	Compiled only when CONFIG_BT_NIMBLE_ENABLED=y. Implements the same public
	API as custom_ble_bluedroid.c so that the LispBM `ble-*` extensions in
	lispif_ble_extensions.c work unchanged on C6.
	...
	Mutating GATT requires no active conn, so add/remove will terminate the
	current peer first if one is connected. This is the cleanest NimBLE
	pattern for dynamic GATT and was approved as the chosen behaviour
	(Stage 3 plan, 2026-05-04).
*/
...
static bool uuid_eq_bd(const esp_bt_uuid_t *a, const esp_bt_uuid_t *b) {
...
static ble_gatt_chr_flags chr_flags_from(esp_gatt_char_prop_t prop, esp_gatt_perm_t perm) {
	ble_gatt_chr_flags f = 0;
	if (prop & ESP_GATT_CHAR_PROP_BIT_READ)      f |= BLE_GATT_CHR_F_READ;
	if (prop & ESP_GATT_CHAR_PROP_BIT_WRITE)     f |= BLE_GATT_CHR_F_WRITE;
	if (prop & ESP_GATT_CHAR_PROP_BIT_WRITE_NR)  f |= BLE_GATT_CHR_F_WRITE_NO_RSP;
	if (prop & ESP_GATT_CHAR_PROP_BIT_NOTIFY)    f |= BLE_GATT_CHR_F_NOTIFY;
	if (prop & ESP_GATT_CHAR_PROP_BIT_INDICATE)  f |= BLE_GATT_CHR_F_INDICATE;
	if (prop & ESP_GATT_CHAR_PROP_BIT_BROADCAST) f |= BLE_GATT_CHR_F_BROADCAST;

	if (perm & ESP_GATT_PERM_READ_ENCRYPTED)     f |= BLE_GATT_CHR_F_READ_ENC;
	if (perm & ESP_GATT_PERM_READ_ENC_MITM)      f |= BLE_GATT_CHR_F_READ_AUTHEN;
	if (perm & ESP_GATT_PERM_WRITE_ENCRYPTED)    f |= BLE_GATT_CHR_F_WRITE_ENC;
	if (perm & ESP_GATT_PERM_WRITE_ENC_MITM)     f |= BLE_GATT_CHR_F_WRITE_AUTHEN;
	if (perm & ESP_GATT_PERM_WRITE_SIGNED)       f |= BLE_GATT_CHR_F_AUTH_SIGN_WRITE;
	return f;
}

static uint8_t dsc_flags_from(esp_gatt_perm_t perm) {
	uint8_t f = 0;
	if (perm & ESP_GATT_PERM_READ)               f |= BLE_ATT_F_READ;
	if (perm & ESP_GATT_PERM_WRITE)              f |= BLE_ATT_F_WRITE;
	if (perm & ESP_GATT_PERM_READ_ENCRYPTED)     f |= BLE_ATT_F_READ_ENC;
	if (perm & ESP_GATT_PERM_READ_ENC_MITM)      f |= BLE_ATT_F_READ_AUTHEN;
	if (perm & ESP_GATT_PERM_WRITE_ENCRYPTED)    f |= BLE_ATT_F_WRITE_ENC;
	if (perm & ESP_GATT_PERM_WRITE_ENC_MITM)     f |= BLE_ATT_F_WRITE_AUTHEN;
	return f;
}
...
static bool get_service_index(uint16_t service_handle, custom_ble_id_t *idx_out) {
...
static struct ble_gatt_svc_def *built_svcs    = NULL;  // size custom_service_len + 1 (terminator)
...
static void free_built_tables(void) {
	free(built_svcs);        built_svcs        = NULL;
	free(built_chrs);        built_chrs        = NULL;
	free(built_dscs);        built_dscs        = NULL;
	free(built_uuids);       built_uuids       = NULL;
	free(built_val_handles); built_val_handles = NULL;
	built_chr_count = 0;
	built_dsc_count = 0;
}

static int gatt_access_cb(uint16_t conn_h, uint16_t attr_h,
...
static int build_service_tables(void) {
	...
	built_svcs        = calloc(custom_service_len + 1, sizeof(struct ble_gatt_svc_def));
	built_chrs        = calloc(total_chrs + custom_service_len, sizeof(struct ble_gatt_chr_def)); // +1 terminator per svc
	built_dscs        = (total_dscs > 0)
		? calloc(total_dscs + total_chrs, sizeof(struct ble_gatt_dsc_def)) // +1 terminator per chr that has dscs
		: NULL;
	...
	for (size_t s = 0; s < custom_service_len; s++) {
		...
		for (size_t a = 0; a < custom_attr_len; a++) {
			...
			svc_chrs[svc_chr_count++] = (struct ble_gatt_chr_def){
				...
				.access_cb  = gatt_access_cb,
				.arg        = &custom_attr[a],
				.flags      = chr_flags_from(custom_attr[a].prop, custom_attr[a].perm),
	...
	switch (ctxt->op) {
		case BLE_GATT_REGISTER_OP_SVC: {
			if (reg_cur_service < custom_service_len) {
				custom_services[reg_cur_service].service_handle = ctxt->svc.handle;
		...
		case BLE_GATT_REGISTER_OP_DSC: {
			...
			if (ctxt->dsc.dsc_def->uuid->type == BLE_UUID_TYPE_16
					&& ((const ble_uuid16_t *)ctxt->dsc.dsc_def->uuid)->value == 0x2902) {
				break;
			}
...
static void start_advertising(void) {
	...
	ble_gap_adv_start(own_addr_type, NULL, BLE_HS_FOREVER, &params,
	                  NULL, NULL); // GAP events use the global cb below
}
...
static int gap_event_cb(struct ble_gap_event *event, void *arg) {
	...
	switch (event->type) {
		...
		case BLE_GAP_EVENT_DISCONNECT:
			is_connected = false;
			LED_BLUE_OFF();
			if (disc_sem) xSemaphoreGive(disc_sem);
			start_advertising_with_cb();
			return 0;
...
static int gatt_access_cb(uint16_t conn_h, uint16_t attr_h,
	...
	if (a == NULL) return BLE_ATT_ERR_UNLIKELY;

	switch (ctxt->op) {
		case BLE_GATT_ACCESS_OP_READ_CHR:
		case BLE_GATT_ACCESS_OP_READ_DSC: {
			if (a->value == NULL || a->value_len == 0) return 0;
		...
		case BLE_GATT_ACCESS_OP_WRITE_DSC: {
			...
			if (in_len > a->value_max_len) in_len = a->value_max_len;
...
static void disconnect_and_wait(void) {
	...
	if (disc_sem) xSemaphoreTake(disc_sem, 0); // drain stale signals
...
static custom_ble_result_t gatt_rebuild(void) {
	if (!has_started || !host_synced) return CUSTOM_BLE_NOT_STARTED;

	disconnect_and_wait();
	ble_gap_adv_stop();

	int rc = ble_gatts_reset();
	if (rc != 0) return CUSTOM_BLE_ESP_ERROR;
...
custom_ble_result_t custom_ble_set_name(const char *name) {
	if (init_result != CUSTOM_BLE_OK) return CUSTOM_BLE_INIT_FAILED;
	if (has_started)                  return CUSTOM_BLE_ALREADY_STARTED;
	size_t len = strlen(name);
	if (len > CUSTOM_BLE_MAX_NAME_LEN) return CUSTOM_BLE_NAME_TOO_LONG;
...
custom_ble_result_t custom_ble_add_service(
	...
	for (size_t i = 0; i < chr_count; i++) {
		...
		if (!buf) goto rollback;
		if (chr[i].value && chr[i].value_len > 0) {
			uint16_t cp = chr[i].value_len;
			if (cp > chr[i].value_max_len) cp = chr[i].value_max_len;
		...
		for (size_t j = 0; j < chr[i].descr_count; j++) {
			...
			if (dbuf_sz == 0) dbuf_sz = 1;
			uint8_t *dbuf = calloc(1, dbuf_sz);
			if (!dbuf) goto rollback;
			if (d->value && d->value_len > 0) {
				uint16_t cp = d->value_len;
				if (cp > d->value_max_len) cp = d->value_max_len;
	...
	if (rb != CUSTOM_BLE_OK) goto rollback;
	...
	if (handles_cb) handles_cb(handles_count, handles);
	free(handles);
	return CUSTOM_BLE_OK;
...
custom_ble_result_t custom_ble_remove_service(uint16_t service_handle) {
	...
	if (svc_idx != custom_service_len - 1) return CUSTOM_BLE_SERVICE_NOT_LAST;
	...
	for (size_t i = 0; i < custom_attr_len; i++) {
		if (custom_attr[i].service_index == svc_idx) {
			if (i < new_attr_len) new_attr_len = i;
...
custom_ble_result_t custom_ble_get_attr_value(
	...
	if (i < 0) return CUSTOM_BLE_INVALID_HANDLE;
	if (length) *length = custom_attr[i].value_len;
	if (value)  *value  = custom_attr[i].value;
	return CUSTOM_BLE_OK;
}
...
custom_ble_result_t custom_ble_set_attr_value(
	...
	if (length > a->value_max_len) length = a->value_max_len;
...
uint16_t custom_ble_get_services(uint16_t capacity, uint16_t service_handles[capacity]) {
	...
	uint16_t n = (custom_service_len < capacity) ? custom_service_len : capacity;
...
int16_t custom_ble_attr_count(uint16_t service_handle) {
	...
	custom_ble_id_t idx;
	if (!get_service_index(service_handle, &idx)) return -1;
	int16_t count = 0;
	for (size_t i = 0; i < custom_attr_len; i++) {
		if (custom_attr[i].service_index == idx) count++;
	}
	return count;
}
...
custom_ble_result_t custom_ble_get_attrs(
	...
	if (!has_started) return CUSTOM_BLE_NOT_STARTED;
	custom_ble_id_t idx;
	if (!get_service_index(service_handle, &idx)) return CUSTOM_BLE_INVALID_HANDLE;
	...
	if (written_count) *written_count = w;
	return CUSTOM_BLE_OK;
}
...
custom_ble_result_t custom_ble_start(void) {
	...
	if (!disc_sem) return CUSTOM_BLE_ERROR;

	esp_err_t err = nimble_port_init();
	if (err != ESP_OK) return CUSTOM_BLE_ESP_ERROR;
...
void custom_ble_init(void) {
	...
	if (service_capacity > 0) {
		...
		if (!custom_services) { init_result = CUSTOM_BLE_ERROR; return; }
	}
	if (chr_descr_capacity > 0) {
		custom_attr = calloc(chr_descr_capacity, sizeof(attr_instance_t));
		if (!custom_attr) { init_result = CUSTOM_BLE_ERROR; return; }
	}
