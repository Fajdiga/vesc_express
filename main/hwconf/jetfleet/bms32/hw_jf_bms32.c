...
static lbm_value ext_bms_init(lbm_value *args, lbm_uint argn) {
	...
	// After RESET the chip reverts to its default I2C address (0x08), so a
	// follow-up subcommand addressed at BQ_ADDR_1 (0x10) is NAK'd. The old
	// SWAP_COMM_MODE call here was a silent no-op for that reason; removed.
...
static lbm_value ext_hw_sleep(lbm_value *args, lbm_uint argn) {
	...
	// gpio_set_level(PIN_PCHG_EN, 0); // CHANGE FROM VBMS32
...
void hw_init(void) {
	...
	gpconf.pin_bit_mask = BIT(PIN_OUT_EN) | BIT(PIN_CHG_EN) | BIT(PIN_COM_EN); // CHANGE FROM VBMS32 gpconf.pin_bit_mask = BIT(PIN_OUT_EN) | BIT(PIN_CHG_EN) | BIT(PIN_PCHG_EN) | BIT(PIN_COM_EN) | BIT(PIN_PSW_EN);
	...
	gpio_set_pull_mode(PIN_PSW_EN, GPIO_FLOATING); // CHANGE FROM VBMS32 -- ADDED
