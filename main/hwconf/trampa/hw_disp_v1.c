...
static lbm_value ext_hw_sleep(lbm_value *args, lbm_uint argn) {
	...
	esp_sleep_enable_gpio_wakeup_on_hp_periph_powerdown(1ULL << pin_btn, ESP_GPIO_WAKEUP_GPIO_HIGH);
#else
	gpio_set_level(pin_bl, 0);
	while (v_btn < 2.0) {
		vTaskDelay(5);
	...
	gpio_set_direction(pin_btn, GPIO_MODE_INPUT);
	esp_sleep_enable_gpio_wakeup_on_hp_periph_powerdown(1ULL << pin_btn, ESP_GPIO_WAKEUP_GPIO_LOW);
#endif
