/*
	NimBLE port of comm_ble.c (VESC Tool BLE channel).

	Compiled only when CONFIG_BT_NIMBLE_ENABLED=y. Implements the same public
	API as comm_ble_bluedroid.c so the rest of the firmware is unchanged.

	GATT layout matches the Bluedroid version exactly (same 128-bit Nordic
	UART-style service / RX / TX UUIDs) so VESC Tool sees an identical
	profile.
*/
 ...
 * the Bluedroid build — see ble_service_uuid128 in comm_ble_bluedroid.c.
 *
